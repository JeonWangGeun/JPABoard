...
public class AttachmentController {
    ...
    public ResponseEntity<Object> download(Long id){
        ...
        if(id == null) return new ResponseEntity<>(null, HttpStatus.BAD_REQUEST);    // 400 error return

        Attachment file = attachmentService.findById(id);

        if(file == null) return new ResponseEntity<>(null, HttpStatus.BAD_REQUEST);    // 400 error return
